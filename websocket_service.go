...
// ACCOUNT WEBSOCKET (PUBLIC)

// WsInstrumentsEvent define websocket struct event
...
// WsInstrumentsHandler handle websocket instrument message
type WsInstrumentsHandler func(event *WsInstrumentsEvent)

// WsInstrumentsServe as per https://www.okex.com/docs-v5/en/#websocket-api-public-channels-instruments-channel
...
// WsMarkPricesEvent define websocket struct event
...
// WsMarkPricesHandler handle websocket instrument message
type WsMarkPricesHandler func(event *WsMarkPricesEvent)

// WsMarkPricesServe as per https://www.okex.com/docs-v5/en/#websocket-api-public-channels-instruments-channel
...
// WsAccountsHandler handle websocket instrument message
...
type WsPositionsEvent struct {
	Arg map[string]string `json:"arg"`
	//Data []*PositionDetail `json:"data"` // todo
}

// WsPositionsHandler handle websocket instrument message
...
// WsOrdersHandler handle websocket instrument message
...
// WsBalancePositionHandler handle websocket PositionBalance message
type WsBalancePositionHandler func(event *WsBalancePositionEvent)

func WsBalancePositionServe(apikey string, secretKey string, passphrase string, handler WsBalancePositionHandler, errHandler ErrHandler, simulated bool) (doneC, stopC chan struct{}, err error) {
	endpoint := getWsEndpoint(true, simulated) // get private endpoint
	return wsBalancePositionServe(endpoint, apikey, secretKey, passphrase, handler, errHandler)
}
